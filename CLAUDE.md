...
as detached, spooled jobs — a small JES2-like job manager. See
**`docs/spec.md`** for the authoritative behavior spec.

## Docs workflow (important)

- **`docs/spec.md`** is the source of truth for behavior. Keep it in sync with code.
- `README.md` is the original brief; `docs/spec.md` supersedes it (and fixes
  its `~/.local/entry` typo → entries live under `~/.local/ges/entry/`).
...
- `supervisor.go` — hidden `__runjob__` supervisor: writes the `sysmsg`
  header/footer, runs+waits on the target so `ges submit` can return
  immediately while still capturing end-of-job stats.
...
- Standard library only — no external deps, except:
  - `github.com/common-nighthawk/go-figure` (used in `supervisor.go` to render
    the JES2-style entry-name banner in the `sysmsg` header).
...
throwaway script — see `examples/` for ready-made ones (including a `dd`
directive example), checked in so users have something to run and read.
